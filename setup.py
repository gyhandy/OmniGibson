...
class CMakeBuild(build_ext):
    def run(self):
        ...
        vivehandtracking_dir = os.path.join(xr_dir, "cpp", "deps", "ViveHandTracking", "libs")
        for f in os.listdir(sranipal_dir):
            if f.endswith("dll"):
                shutil.copy(os.path.join(sranipal_dir, f), xr_dir)
        for f in os.listdir(vivehandtracking_dir):
            if f.endswith("dll"):
                shutil.copy(os.path.join(vivehandtracking_dir, f), xr_dir)
